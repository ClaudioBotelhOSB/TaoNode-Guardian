#!/bin/bash
# =============================================================================
# TaoNode Guardian -- K3s Bootstrap Script (GCP)
# =============================================================================
# Executed as root via GCE metadata_startup_script on first boot.
# Output: /var/log/syslog (or journalctl -u google-startup-scripts)
#
...
# GHCR_PAT is injected by Terraform via metadata_startup_script templating.
...
# -- Summary -------------------------------------------------------------------
PUBLIC_IP=$(curl -sfH "Metadata-Flavor: Google" \
  http://metadata.google.internal/computeMetadata/v1/instance/network-interfaces/0/access-configs/0/external-ip || echo "<public-ip>")

log "============================================================"
log "Bootstrap complete! ArgoCD is now reconciling all workloads."
log ""
log "  Kubeconfig (copy from remote):"
log "    scp -i ~/.ssh/taonode-demo ubuntu@${PUBLIC_IP}:/etc/rancher/k3s/k3s.yaml ~/.kube/taonode-gcp.yaml"
