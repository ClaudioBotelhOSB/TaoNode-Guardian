#!/bin/bash
# =============================================================================
# TaoNode Guardian — K3s Bootstrap Script (OVH Public Cloud)
# =============================================================================
# Executed as root via user_data / cloud-init on first boot.
...
# --tls-san adds the public IP to the K3s TLS certificate so that kubectl
# from outside the instance works without --insecure-skip-tls-verify.
log "STEP 2: Fetching public IP for TLS SAN"
# OVH metadata endpoint may differ from AWS; try multiple methods:
PUBLIC_IP=$(
  curl -sf --max-time 5 http://169.254.169.254/latest/meta-data/public-ipv4 2>/dev/null ||
  curl -sf --max-time 5 http://169.254.169.254/openstack/latest/meta_data.json 2>/dev/null | jq -r '.meta.public_ip // empty' ||
...
log "STEP 2: Installing K3s (tls-san: ${PUBLIC_IP:-none})"
EXEC_ARGS="server --disable traefik --write-kubeconfig-mode 644"
[[ -n "$PUBLIC_IP" ]] && EXEC_ARGS="${EXEC_ARGS} --tls-san ${PUBLIC_IP}"
...
# ── Summary ───────────────────────────────────────────────────────────────────
# Re-fetch public IP for the summary (may have changed or become available now)
SUMMARY_IP=$(
  curl -sf --max-time 5 http://169.254.169.254/latest/meta-data/public-ipv4 2>/dev/null ||
  curl -sf --max-time 5 http://169.254.169.254/openstack/latest/meta_data.json 2>/dev/null | jq -r '.meta.public_ip // empty' ||
  curl -sf --max-time 5 https://api.ipify.org 2>/dev/null ||
...
log "    scp -i ~/.ssh/taonode-demo ubuntu@${SUMMARY_IP}:/etc/rancher/k3s/k3s.yaml ~/.kube/taonode-ovh.yaml"
log "    export KUBECONFIG=~/.kube/taonode-ovh.yaml"
log ""
...
log ""
log "  Access URLs (Klipper LoadBalancer):"
log "    Grafana    -> http://${SUMMARY_IP}:3000"
log "    ArgoCD     -> http://${SUMMARY_IP}:8080"
log "    ClickHouse -> http://${SUMMARY_IP}:8123/play"
log ""
