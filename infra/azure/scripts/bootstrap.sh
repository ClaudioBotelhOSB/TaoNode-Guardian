#!/bin/bash
# =============================================================================
# TaoNode Guardian — K3s Bootstrap Script (Azure)
# =============================================================================
# Executed as root via Azure custom_data / cloud-init on first boot.
...
# GHCR_PAT is injected by Terraform via custom_data templating.
...
# --tls-san adds the VM public IP to the K3s TLS certificate so that kubectl
# from outside the VM works without --insecure-skip-tls-verify.
log "STEP 2: Fetching Azure VM public IP for TLS SAN"
VM_PUBLIC_IP=$(curl -sH "Metadata:true" "http://169.254.169.254/metadata/instance/network/interface/0/ipv4/ipAddress/0/publicIpAddress?api-version=2021-02-01&format=text" || echo "")
if [[ -z "$VM_PUBLIC_IP" ]]; then
  log "WARNING: could not fetch public IP from metadata — kubectl from outside will require --insecure-skip-tls-verify"
fi

log "STEP 2: Installing K3s (tls-san: ${VM_PUBLIC_IP:-none})"
EXEC_ARGS="server --disable traefik --write-kubeconfig-mode 644"
[[ -n "$VM_PUBLIC_IP" ]] && EXEC_ARGS="${EXEC_ARGS} --tls-san ${VM_PUBLIC_IP}"
...
# -- Summary -------------------------------------------------------------------
PUBLIC_IP=$(curl -sH "Metadata:true" "http://169.254.169.254/metadata/instance/network/interface/0/ipv4/ipAddress/0/publicIpAddress?api-version=2021-02-01&format=text" || echo "<public-ip>")

log "============================================================"
log "Bootstrap complete! ArgoCD is now reconciling all workloads."
log ""
log "  Kubeconfig (copy from remote):"
log "    scp -i ~/.ssh/taonode-demo ubuntu@${PUBLIC_IP}:/etc/rancher/k3s/k3s.yaml ~/.kube/taonode-azure.yaml"
log "    export KUBECONFIG=~/.kube/taonode-azure.yaml"
log ""
