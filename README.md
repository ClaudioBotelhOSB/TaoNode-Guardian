...
The operator implements a rigorous 10-step reconciliation loop conforming to the `controller-runtime` `Reconciler` interface, safeguarded by proactive `webhook` validation logic to block misconfigurations before etcd insertion. The workload lifecycle follows this flow:

```
Reconcile(ctx, req)
 ├── 1. Fetch TaoNode CR
 ├── 2. Finalizer management (tao.guardian.io/finalizer)
 ├── 3. Ensure Workload resources (StatefulSet Builder, Service, ServiceAccount)
...
**RBAC — Segregated Least Privilege Topology:**

Instead of a monolithic ClusterRole, the operator implements a rigorous 5-role segregation model to minimize the blast radius in the event of an operator compromise:

| Role | Scope | Purpose & Key Restrictions |
|------|-------|----------------------------|
| **CRD Manager** | ClusterRole | Discovers `taonodes`, updates `status` and `finalizers`. (Cannot create/delete CRs). |
| **Resource Observer** | ClusterRole | Read-only discovery for `nodes` (for scheduling affinity) and `namespaces`. |
| **Leader Election** | Role | Namespace-scoped (`taonode-system`). Manages `leases` for controller HA. |
| **Webhook Certs** | Role | Namespace-scoped (`taonode-system`). `get` access strictly tied to `webhook-server-cert`. |
| **Workload Manager** | Role | Namespace-scoped strictly to managed subnets (e.g. `bittensor`). Manages `statefulsets`, `services`, `pvcs`, `volumesnapshots`, etc. **Crucially**, it reads Secret metadata only (cannot extract key content) and cannot mutate pods in unauthorized namespaces. |
...
Continuous security is deeply embedded into the development workflow and CI/CD parameters:
- **Pre-commit Hooks:** Proactively blocks commits containing plain-text keys via `Gitleaks`.
- **CI Pipeline (`security.yml`):** Enforces automated static analysis on every Pull Request:
  - `gosec`: Golang AST security auditing (detects unsafe pointers, weak cryptos, SQLi).
  - `checkov`: Scans Infrastructure as Code and Kubernetes manifests for structural flaws.
  - `trivy`: Operates at the container image level, searching for known CVEs.
  - `gitleaks`: Re-evaluates historical and push-level secret scanning.
